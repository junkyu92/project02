...
<!-- 우측 상단 div (bgm/도토리/검색) -->
	...
	<div id="frame">
		...
		<div id="mainLeft">
			...
			<div id="diaryLeftBook">
				<c:forEach var="list" items="${diarybook}">
					<a href="diaryPageView?memid=${sessionId}&bookid=${list.id}">${list.title}</a>
					<br>
				</c:forEach>
			</div>
		</div>
		...
		<div id="mainRight">
			<form>
				<c:forEach var="list" items="${diarypage}">
					<hr>
					제목:${list.title} <br>
					내용:${list.content}<br>

					<c:set var="TextValue" value="${list.date}" />
					날짜:${fn:substring(TextValue,0,10)} <br>
					
					<a href="diaryPageUpdateView?memid=${sessionId}&pageid=${list.id}&bookid=<%=request.getParameter("bookid")%>">
					수정</a>
					<a href="diaryPageDeleteAction?memid=${sessionId}&pageid=${list.id}&bookid=<%=request.getParameter("bookid")%>">
					삭제</a>
					<hr>
				</c:forEach>
					<a
					href="diaryPageWriteView?memid=${sessionId}&bookid=<%=request.getParameter("bookid")%>">
					쓰기</a>
					
					<a
					href="diaryBookSetView?memid=${sessionId}&bookid=<%=request.getParameter("bookid")%>">
					설정</a>
			</form>
		</div>
	</div>
